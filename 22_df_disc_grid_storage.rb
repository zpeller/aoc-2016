...
input = (ARGV.empty? ? DATA : ARGF).each_line.drop(2).map { |l|
	nums = l.scan(/-?\d+/).map(&:to_i)
#	["node-x#{nums[0]}-y#{nums[1]}", nums[2], nums[3], nums[4], nums[5]]
	[nums[0], nums[1], nums[2], nums[3], nums[4], nums[5]]
}.freeze

def init_disk_hash(disks, stones, input)
	zero = []
	max_x = 0
	input.each {|x, y, size, used, avail, perc|
		disks[[x,y]] = [size, used, avail, perc]
		stones << [x,y] if size > 200
		zero = [x,y] if used == 0
		max_x = [max_x, x].max
	}
	return zero, max_x
...
def num_viable_disks(disks)
	disks.keys.permutation(2).select {|nodeA, nodeB| 
		disks[nodeA][1]>0 and disks[nodeA][1] <= disks[nodeB][2]
	}.length
end
...
# ..........^............
# ..........^<.<.<.0.....
def steps_needed(zero, max_x, stones)
	steps = 0

	wall_left_x = stones.sort[0][0]

	while zero[0] >= wall_left_x
		steps += 1
		zero[0] -= 1
	end
...
stones = []
max_x = 0
zero, max_x = init_disk_hash(disks, stones, input)

print("P1 num viable pairs: #{num_viable_disks(disks)}/#{disks.length}\n")

#print zero, " ", stones, "\n"

print("P2 minimum steps needed: #{steps_needed(zero, max_x, stones)}\n")
