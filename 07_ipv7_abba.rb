...
def supports_tls(s)
	s.scan(/\[[a-z]*([a-z])([a-z])\2\1/).each { |c1, c2|
		return false if c1 != c2
	}
	s.scan(/([a-z])([a-z])\2\1/).each { |c1, c2|
...
def supports_ssl(s)
	bracket_strings = s.scan(/\[([a-z]*)\]/)

	if bracket_strings.empty?
		return false
	end

	remove_re = Regexp.new('\[(' + bracket_strings.join('|') + ')\]')
	bracketless_s = s.gsub(remove_re, '-')
#	print bracketless_s, "\n"

	bracket_strings.join('-')
					.scan(/(?=(.)(.)\1)/)			# ?= positive lookahead regex, overlapping matches! (backward)
					.select { |c1, c2| c1!=c2 }
					.map{ |c1, c2| c2+c1+c2 }
					.each { |pat| 
#						print pat, "\n"
						if bracketless_s =~ Regexp.new(pat)
							return true
						end
					}
	return false
...
print("P1 num TLS IP-s: #{tls_sup}\n")
print("P2 num SSL IP-s: #{ssl_sup}\n")
