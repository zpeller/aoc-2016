...
input = (ARGV.empty? ? DATA : ARGF).each_line.map { |l|
	if l =~ /^value/
		...
		tokens = l.strip.scan(/bot (\d+) gives low to (.+) (\d+) and high to (.+) (\d+)$/)[0].freeze
		['bot', tokens[0].to_i, tokens[1], tokens[2].to_i, tokens[3], tokens[4].to_i]
	end
}.freeze

class Bot 
	def initialize(bot_id, low_output_type, low_output_id, high_output_type, high_output_id)
		@bot_id, @low_output_type, @low_output_id, @high_output_type, @high_output_id = bot_id, low_output_type, low_output_id, high_output_type, high_output_id
		@values = []
	end
	...
	def append(value)
		@values += [value]
		if @values.length == 2
			if [17, 61] == @values.sort
				print("P1 17-61 bot: #{@bot_id}\n")
			end
			if @low_output_type=='bot'
				$bots[@low_output_id].append(@values.min)
			else
				$outputs[@low_output_id].append(@values.min)
			end
			if @high_output_type=='bot'
				$bots[@high_output_id].append(@values.max)
			else
				$outputs[@high_output_id].append(@values.max)
			end
			@values = []
		end
	end
	...
	def inspect
		"#{@bot_id} v#{@low_output_type}:#{@low_output_id} ^#{@high_output_type}:#{@high_output_id} #{@values}"
	end
...
def init_bots(input)
	input.select {|type, *arg| type == "bot"}.each {|type, bot_id, low_type, low_id, high_type, high_id|
		$bots[bot_id] = Bot.new(bot_id, low_type, low_id, high_type, high_id)
		if low_type == 'output'
			$outputs[low_id] = Output.new(low_id)
		end
		if high_type == 'output'
			$outputs[high_id] = Output.new(high_id)
		end
	}

	input.select {|type, *arg| type == "value"}.each {|type, bot_id, value|
		$bots[bot_id].append(value)
	}

#	$bots.each { |bot| print bot, "\n" }
#	$outputs.each { |x| print x, "\n" }
end
...
print("P2 output1*2*3: #{$outputs[0].values[0]*$outputs[1].values[0]*$outputs[2].values[0]}\n")
