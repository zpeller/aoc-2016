...
input = (ARGV.empty? ? DATA : ARGF).each_line.map { |l|
	tokens = l.strip.scan(/^(.*)-(\d+)\[(.*)\]$/)[0].freeze
	[tokens[0], tokens[1].to_i, tokens[2]]
}.freeze

def five_most_common_letters(s)
	return s.tr('-', '')
		.chars
		.group_by(&:itself)
		.map{ |letter, letters| [letters.count, letter] }
		.sort{ |a,b| if a[0]==b[0]; a[1]<=>b[1]; else b[0]<=>a[0]; end }
		.first(5)
		.map{ |x| x[1] }
		.join
end
...
valid_rooms = input.select { |name, sector_id, cksum|
					five_most_common_letters(name)==cksum
}
num_valid = valid_rooms.inject(0) { |sum, l| sum + l[1] }

print("P1 valid room names: #{num_valid}\n")

valid_rooms.each { |name, sector_id, cksum|
	if decrypt_name(name, sector_id) == 'northpole object storage'
		print("P2 'northpole object storage' sector id: #{sector_id}\n")
	end
}
